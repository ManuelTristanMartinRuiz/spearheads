...
materiales_conservacion <-table(spear$Materiales, spear$Conservación)
...
procentaje_conservacion <- prop.table(table(spear$Conservación)) * 100
...
porcentaje_materiales_conservacion <- prop.table(table(spear$Materiales, spear$Conservación), margin = 1) * 100
...
#Ej.8
barras_verticales_conservacion <- barplot(table(spear$Conservación), main = "Frecuencia de Conservación", 
                                          xlab = "Conservación")
...
#Para elaborar estos graficos utilizamos la misma función que en Ej.8 pero añadimos la función "horiz" = True para que los haga orizontales.

#Ej.10
barras_material_conservacion <- barplot(table(spear$Conservación, spear$Materiales), beside = TRUE, legend = TRUE, main = "Conservación por Material", xlab = "Conservación", col = c("red", "blue"), names.arg = c("Excelente", "Bueno", "Regular", "Malo"))
...
#Ej.11
sectores_conservacion <- pie(table(spear$Conservación), main = "Porcentaje de Conservación")
